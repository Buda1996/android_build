...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^epic_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $EPIC_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       EPIC_BUILD=
    fi
    export EPIC_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    EPIC_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/epic/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Epic-OS github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var EPIC_VERSION)
        ZIPFILE=epic-os-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.epic.device=$EPIC_BUILD");
    then
...
function cmremote()
{
    ...
    EPICUSER=$(git config --get review.review.cyanogenmod.org.username)
    if [ -z "$EPICUSER" ]
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/epic/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
